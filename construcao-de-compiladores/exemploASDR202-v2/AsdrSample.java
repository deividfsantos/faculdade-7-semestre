 import java.io.*;

public class AsdrSample {

  private static final int BASE_TOKEN_NUM = 301;
  
  public static final int IDENT  = 301;
  public static final int NUM 	 = 302;
  public static final int WHILE  = 303;
  public static final int IF	 = 304;
  public static final int FI	 = 305;
  public static final int ELSE = 306;
  public static final int INT	 = 305;
  public static final int DOUBLE = 306;

    public static final String tokenList[] = 
      {"IDENT",
		 "NUM", 
		 "WHILE", 
		 "IF", 
		 "FI",
		 "ELSE"  };
                                      
  /* referencia ao objeto Scanner gerado pelo JFLEX */
  private Yylex lexer;

  public ParserVal yylval;

  private static int laToken;
  private boolean debug;

  
  /* construtor da classe */
  public AsdrSample (Reader r) {
      lexer = new Yylex (r, this);
  }

  /***** Gramática original 
  Prog -->  LDecl Bloco

  LDecl --> Decl LDecl
        |      // vazia

  Decl --> Tipo LIdent ;

  LIdent --> ident
            | LIdent , ident    // precisa eliminar rec a esquerda

  Tipo --> int | double

  Bloco --> { LCmd }

  LCmd --> Cmd  LCmd
          |    //vazio   

  Cmd --> Bloco
      | while ( E ) Cmd
      | ident = E ;
      | if ( E ) Cmd 
      | if ( E ) Cmd else Cmd 
//
//  E --> E+E | E-E | E*E | E/E | E%E | ident | num | (E)
//       não pode ser usadada porque é ambigua
//       abaixo a versão não ambígua
//       - um conj de prod por nível de precedência
//       - recursão à esquerda para operadores assoc à esquerda

  E --> E + T
       | E - T
       | T
  
  T --> T * F
       | T / F
       | T % F
       | F

F -->   IDENT
   | NUM
   | ( E )
***/  

  /***** Gramática 'fatorada' 
  Prog -->  LDecl Bloco

  LDecl --> Decl LDecl
        |      // vazia

  Decl --> Tipo LIdent ;

  LIdent --> ident RestoLIdent

  RestoLIdent -->  , ident RestoLIdent
                |         // vazio

  Tipo --> int | double

  Bloco --> { LCmd }

  LCmd --> Cmd  LCmd
          |    //vazio  
  
  Cmd --> Bloco
      | while ( E ) Cmd
      | ident = E ;
      | if ( E ) Cmd RestoIf   // 'fatorada à esquerda'
      
   RestoIf --> else Cmd 
            |               // producao vazia

  // eliminando a recursão a esquerda...

  E --> T R
  R --> + T R
       | - T R
       |     // vazio

  T --> F S
  S --> * F S
      | / F S
      | % F S
      |     // vazio
      
F -->  IDENT
   | NUM
   | ( E )
***/ 

  private void Prog() {
   
      if (laToken == '{') {
         if (debug) System.out.println("Prog --> Bloco");
         Bloco();
      }
      else 
        yyerror("esperado '{'");
   }

//   LDecl --> Decl LDecl
//   |      // vazia

   private void LDecl() {
      if (laToken == INT || laToken == DOUBLE ) {     // if la pertence a first('decl ldecl')
      if (debug) System.out.println("LDecl --> Tipo LIdent ;"); 
         Tipo();
          LIdent();
          verifica(';');   
      } else {     // producao vazia
         if (debug) System.out.println("LDecl -->   /*vazio*/");

      }
   }

  private void Bloco() {
      if (debug) System.out.println("Bloco --> { Cmd }");

      verifica('{');
      Cmd();
	   verifica('}');
  }

  private void Cmd() {
      if (laToken == '{') {
         if (debug) System.out.println("Cmd --> Bloco");
         Bloco();
	   }    
      else if (laToken == WHILE) {
         if (debug) System.out.println("Cmd --> WHILE ( E ) Cmd");
         verifica(WHILE);    // laToken = this.yylex(); 
  		   verifica('(');
  		   E();
         verifica(')');
         Cmd();
	   }
      else if (laToken == IDENT ) {
         if (debug) System.out.println("Cmd --> IDENT = E ;");
            verifica(IDENT);  
            verifica('='); 
            E();
		      verifica(';');
	   }
    else if (laToken == IF) {
         if (debug) System.out.println("Cmd --> if (E) Cmd RestoIF");
         verifica(IF);
         verifica('(');
  		   E();
         verifica(')');
         Cmd();
         RestoIF();
	   }
 	else yyerror("Esperado {, if, while ou identificador");
   }


   private void RestoIF() {
       if (laToken == ELSE) {
         if (debug) System.out.println("RestoIF --> else Cmd ");
         verifica(ELSE);
         Cmd();
         //verifica(FI);
    
	   }// else if (laToken == FI){
        // if (debug) System.out.println("RestoIF -->  FI  ");
        // verifica(FI); 
        // }
        // else yyerror("Esperado else ou fi");
        else {
            // produção vazia 
            // opção 1. verifica o conjunto Follow e dá msg de erro
            // opção 2. usa como regra default (faz nada)
            if (debug) System.out.println("RestoIF -->  /*vazio*/ ");
        }
     }     



  private void E() {
      if (laToken == IDENT) {
         if (debug) System.out.println("E --> IDENT");
         verifica(IDENT);
	   }
      else if (laToken == NUM) {
         if (debug) System.out.println("E --> NUM");
         verifica(NUM);
	   }
      else if (laToken == '(') {
         if (debug) System.out.println("E --> ( E )");
         verifica('(');
         E();        
		 verifica(')');
	   }
 	else yyerror("Esperado operando (, identificador ou numero");
   }


  private void verifica(int expected) {
      if (laToken == expected)
         laToken = this.yylex();
      else {
         String expStr, laStr;       

		expStr = ((expected < BASE_TOKEN_NUM )
                ? ""+(char)expected
			     : tokenList[expected-BASE_TOKEN_NUM]);
         
		laStr = ((laToken < BASE_TOKEN_NUM )
                ? new Character((char)laToken).toString()
                : tokenList[laToken-BASE_TOKEN_NUM]);

          yyerror( "esperado token : " + expStr +
                   " na entrada: " + laStr);
     }
   }

   /* metodo de acesso ao Scanner gerado pelo JFLEX */
   private int yylex() {
       int retVal = -1;
       try {
           yylval = new ParserVal(0); //zera o valor do token
           retVal = lexer.yylex(); //le a entrada do arquivo e retorna um token
       } catch (IOException e) {
           System.err.println("IO Error:" + e);
          }
       return retVal; //retorna o token para o Parser 
   }

  /* metodo de manipulacao de erros de sintaxe */
  public void yyerror (String error) {
     System.err.println("Erro: " + error);
     System.err.println("Entrada rejeitada");
     System.out.println("\n\nFalhou!!!");
     System.exit(1);
     
  }

  public void setDebug(boolean trace) {
      debug = true;
  }


  /**
   * Runs the scanner on input files.
   *
   * This main method is the debugging routine for the scanner.
   * It prints debugging information about each returned token to
   * System.out until the end of file is reached, or an error occured.
   *
   * @param args   the command line, contains the filenames to run
   *               the scanner on.
   */
  public static void main(String[] args) {
     AsdrSample parser = null;
     try {
         if (args.length == 0)
            parser = new AsdrSample(new InputStreamReader(System.in));
         else 
            parser = new  AsdrSample( new java.io.FileReader(args[0]));

          parser.setDebug(false);
          laToken = parser.yylex();          

          parser.Prog();
     
          if (laToken== Yylex.YYEOF)
             System.out.println("\n\nSucesso!");
          else     
             System.out.println("\n\nFalhou - esperado EOF.");               

        }
        catch (java.io.FileNotFoundException e) {
          System.out.println("File not found : \""+args[0]+"\"");
        }
//        catch (java.io.IOException e) {
//          System.out.println("IO error scanning file \""+args[0]+"\"");
//          System.out.println(e);
//        }
//        catch (Exception e) {
//          System.out.println("Unexpected exception:");
//          e.printStackTrace();
//      }
    
  }
  
}

